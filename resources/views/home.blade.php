...
@section('content')
    <div class="bg-white shadow-lg mt-4 mb-4 w-[430px] rounded-lg">
        <h1 class="text-3xl font-semibold p-3">Selamat Datang {{ Auth::user()->name }}</h1>    
    </div>
    <div>
        <div class="inline-flex bg-white shadow-lg p-3 w-52 rounded-lg relative">
            <div>
                <p>Jumlah Pesanan:</p>
                <p class="text-3xl font-semibold">{{ $pesanancount }}</p>    
            </div>
            <svg class="absolute right-3 mt-2" xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" viewBox="0 0 24 24"><path fill="#e2e8f0" d="m17.275 20.25l3.475-3.45l-1.05-1.05l-2.425 2.375l-.975-.975l-1.05 1.075zM6 9h12V7H6zm12 14q-2.075 0-3.537-1.463T13 18t1.463-3.537T18 13t3.538 1.463T23 18t-1.463 3.538T18 23M3 22V5q0-.825.588-1.412T5 3h14q.825 0 1.413.588T21 5v6.675q-.7-.35-1.463-.513T18 11H6v2h7.1q-.425.425-.787.925T11.675 15H6v2h5.075q-.05.25-.062.488T11 18q0 1.05.288 2.013t.862 1.837L12 22l-1.5-1.5L9 22l-1.5-1.5L6 22l-1.5-1.5z"/></svg>
        </div>
    ...
    <div class="inline-flex mt-4">
        ...
        <div class="w-52 h-full bg-white shadow-lg ml-4 rounded-lg">
            <h1 class="font-bold p-3">Menu Terbaru</h1>
            @foreach ($menu->slice(0,2) as $menudata)
                <div class="bg-white m-3 rounded-lg p-3 shadow-lg">
                    <img class="h-32 w-full rounded-lg" src="{{ $menudata->foto }}" alt="menu">
